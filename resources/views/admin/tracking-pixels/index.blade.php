...
@section('title', $title ?? 'Tracking Pixels')

@section('content_header')
<div class="d-flex justify-content-between align-items-center flex-wrap">
    <div>
        <h1 class="mb-0">{{ $title ?? 'Tracking Pixels' }}</h1>
...
<div class="card shadow-sm border-0" style="border-radius: 12px;">
    <div class="card-header bg-white py-3 border-0">
        <div class="row align-items-center">
            ...
            <div class="col-md-6 mt-2 mt-md-0 text-md-right">
                ...
                <a href="{{ route('admin.tracking-pixels.create') }}"
                   class="btn btn-primary btn-sm px-4 shadow-sm ml-2"
                   id="btnAddPixel"
                   style="border-radius: 8px;">
                    <i class="fas fa-plus-circle mr-1"></i> Add Pixel
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="card-body p-0">
        ...
        <div class="px-4 py-3 border-top bg-white">
            <div class="row">
                <div class="col-md-3 col-sm-6 mb-2">
                    <label class="small font-weight-bold text-muted text-uppercase">Platform</label>
                    <select id="filter_platform" class="form-control border-0 bg-light shadow-none">
                ...
                <div class="col-md-4 col-sm-8 mb-2">
                    ...
                    <input type="text"
                           ...
                           placeholder="Search name, pixel ID or platform...">
                </div>
        ...
        <div id="content-wrapper" style="min-height: 400px; position: relative;">
            @include('admin.tracking-pixels.partials.table', [
...
$(document).ready(function() {
    ...
    function getBaseUrl() {
        return currentView === 'trash'
            ? "{{ route('admin.tracking-pixels.trashed') }}"
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status && res.html) {
                    ...
                    showToast('error', 'Failed to fetch tracking pixels.');
                }
            },
            error: function() {
                $('#content-wrapper').css('opacity', '1');
    ...
    function updateUIState() {
        if (currentView === 'trash') {
            ...
            $('#btnAddPixel').removeClass('d-none');
        }
    }

    $('#filter_platform, #filter_status').on('change', function() {
        reloadTable(1);
    });
    ...
    $(document).on('click', '.btnDelete, .btnRestore, .btnForceDelete', function() {
        ...
        Swal.fire({
            title: isRestore ? 'Restore pixel?' : (isForce ? 'Purge forever?' : 'Move to trash?'),
    ...
    $('#btnApplyBulk').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `Action: ${action} on ${ids.length} tracking pixels.`,
            icon: 'warning',
            type: 'warning',
            showCancelButton: true
        }).then((result) => {
            if (result.isConfirmed || result.value) {
                $.ajax({
                    url: "{{ route('admin.tracking-pixels.multiple_action') }}",
                    type: "POST",
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: ids,
