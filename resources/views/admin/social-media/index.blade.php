...
@section('title', $title ?? 'Social Media')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap">
        <div>
            <h1 class="mb-0">{{ $title ?? 'Social Media Links' }}</h1>
            @if(isset($breadcrumb))
        ...
        <div>
            @if(isset($isTrash) && $isTrash)
                <a href="{{ route('admin.social-media.index') }}" class="btn btn-outline-primary btn-sm shadow-none">
                    <i class="fas fa-list mr-1"></i> Active Links
                </a>
            @else
                <a href="{{ route('admin.social-media.trashed') }}" class="btn btn-outline-danger btn-sm shadow-none mr-2">
                    <i class="fas fa-trash-alt mr-1"></i> Trash Bin
                </a>
                <button type="button" class="btn btn-primary btn-sm shadow-none btnModal" data-url="{{ route('admin.social-media.create') }}">
    ...
    <div class="card shadow-sm border-0" style="border-radius: 12px;">
        ...
        <div class="card-body p-0">
            ...
            <div class="px-4 py-3 border-top bg-white">
                <div class="row">
                    ...
                    <div class="col-md-7 col-sm-12 mb-2">
                        ...
                        <input type="text" id="table_search" class="form-control shadow-none" placeholder="Search by platform name or link...">
                    </div>
            ...
            <div id="content-wrapper" style="min-height: 300px; position: relative;">
                @include('admin.social-media.partials.table', ['socialMedias' => $socialMedias, 'isTrash' => $isTrash ?? false])
            </div>
        </div>
    </div>
...
@section('js')
    <script>
        $(document).ready(function () {
            ...
            // Submit AJAX Form (No File Upload needed for Social Media)
            ...
            $(document).on('click', '.btnAction', function () {
                ...
                let title = actionType === 'restore' ? 'Restore Link?' : (actionType === 'force_delete' ? 'Delete Permanently?' : 'Move to Trash?');
            ...
            $('#btnApplyBulk').click(function () {
                ...
                Swal.fire({
                    ...
                    if (result.value || result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.social-media.multiple_action') }}",
                            type: "POST",
