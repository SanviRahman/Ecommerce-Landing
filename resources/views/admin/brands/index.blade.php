            ...
            <div class="px-4 py-3 border-top bg-white">
                <div class="row">
                    ...
                    <div class="col-md-7 col-sm-12 mb-2">
                        ...
                        <input type="text" id="table_search" class="form-control shadow-none" placeholder="Search brand name or slug...">
                    </div>
        ...
        $(document).ready(function () {
            ...
            function updateUIState() {
                if (currentView === 'trash') {
                    ...
                    $('#btnToggleTrash').html('<i class="fas fa-list mr-1"></i> Active List').removeClass('btn-outline-danger').addClass('btn-outline-primary');
                    ...
                    $('#btnToggleTrash').html('<i class="fas fa-trash-alt mr-1"></i> Trash Bin').removeClass('btn-outline-primary').addClass('btn-outline-danger');
            ...
            $('#btnAddBrand').on('click', function () {
                $.ajax({
                    ...
                    type: "GET",
                    dataType: "json",
                    beforeSend: function() { $('#btnAddBrand').prop('disabled', true); },
                    complete: function() { $('#btnAddBrand').prop('disabled', false); },
                    success: function (res) {
                        if (res.status && res.html) {
                    ...
                    error: function() { showToast('error', 'Form load failed.'); }
                });
            });
            ...
            $(document).on('click', '.btnDelete, .btnRestore, .btnForceDelete', function () {
                ...
                Swal.fire({
                    title: isRestore ? 'Restore brand?' : (isForce ? 'Purge forever?' : 'Move to trash?'),
