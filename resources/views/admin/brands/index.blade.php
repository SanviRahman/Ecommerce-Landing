...
@section('title', $title ?? 'Brand Management')

@section('content_header')
<div class="d-flex justify-content-between align-items-center flex-wrap">
    <div>
        <h1 class="mb-0">{{ $title ?? 'Brand Management' }}</h1>
    ...
    <div class="card-header bg-white py-3 border-0">
        <div class="row align-items-center">
            ...
            <div class="col-md-6 mt-2 mt-md-0 text-md-right">
                ...
                <button class="btn btn-primary btn-sm px-4 shadow-sm ml-2" id="btnAddBrand" style="border-radius: 8px;">
    ...
    <div class="modal-dialog modal-md modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
            <div class="modal-header border-0 pb-0">
                <h6 class="modal-title font-weight-bold text-primary">Brand Configuration</h6>
...
@section('footer')
<strong>
    © Copyright 2026 All rights reserved | This website developed by <a href="https://sfashanto.netlify.app/"
        target="_blank">SFA Shanto</a>
</strong>
@endsection
...
$(document).ready(function() {
    ...
    function updateUIState() {
        if (currentView === 'trash') {
            ...
            $('#btnToggleTrash').html('<i class="fas fa-list mr-1"></i> Active List').removeClass(
                'btn-outline-danger').addClass('btn-outline-primary');
            $('#btnAddBrand').addClass('d-none');
        } else {
            ...
            $('#btnToggleTrash').html('<i class="fas fa-trash-alt mr-1"></i> Trash Bin').removeClass(
                'btn-outline-primary').addClass('btn-outline-danger');
            $('#btnAddBrand').removeClass('d-none');
        }
    }
    ...
    $('#btnAddBrand').on('click', function() {
        $.ajax({
            ...
            dataType: "json",
            beforeSend: function() {
                $('#btnAddBrand').prop('disabled', true);
            },
            complete: function() {
                $('#btnAddBrand').prop('disabled', false);
            },
            success: function(res) {
                if (res.status && res.html) {
            ...
            error: function() {
                showToast('error', 'Form load failed.');
            }
        });
    });
    ...
    $(document).on('click', '.btnDelete, .btnRestore, .btnForceDelete', function() {
        ...
        Swal.fire({
            title: isRestore ? 'Restore brand?' : (isForce ? 'Purge forever?' :
                'Move to trash?'),
    ...
    $('#btnApplyBulk').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `Action: ${action} on ${ids.length} brands.`,
            icon: 'warning',
            type: 'warning', // এটি v8 এর জন্য যোগ করা হলো
            showCancelButton: true,
            confirmButtonText: 'Yes, Proceed' // কনফার্ম বাটনের টেক্সট
        }).then((result) => {
            // v8 এবং v9+ উভয়ের জন্য সাপোর্ট
            if (result.isConfirmed || result.value) {
                $.ajax({
                    url: "{{ route('admin.brands.multiple_action') }}",
                    type: "POST",
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: ids,
